...
Let's test the correlation of each of the four independent variables in Model 7.
```{r final correlation tests}
print("SLOPE")
cor(Dat_H$CHNG_URB, Dat_H$SLOPE, use="complete.obs", method="pearson")
print("REGRAIL300")
cor(Dat_H$CHNG_URB, Dat_H$REGRAIL300, use="complete.obs", method="pearson")
print("DIST_INTER")
cor(Dat_H$CHNG_URB, Dat_H$DIST_INTER, use="complete.obs", method="pearson")
print("DIST_PARKS")
cor(Dat_H$CHNG_URB, Dat_H$DIST_PARKS, use="complete.obs", method="pearson")
```
```{r predict and plot}
...
This yields are final list of independent variables to include in our models:
...
mod_A <- glm ( drivealone ~ AGE + GEND_F + TRPDUR + INCOM + TOLL + PARKC +
                PARKU_H + PARKU_D + PARKU_W + PARKU_M,
...
We run a new model with just `GEND_F`, `TRPDUR`, `INCOM`, and `TOLL` based on these results.

```{r B1 mod2}
mod_B <- glm ( drivealone ~  GEND_F + TRPDUR + INCOM + TOLL,
               data=CommuteTripsPerson, family = binomial)
summary(mod_B)
```
...
```{r r B1 mod3}
mod_C <- glm ( drivealone ~ GEND_F + TRPDUR + INCOM, data=CommuteTripsPerson, family = binomial)
summary(mod_C)
```
...
```{r mod plots}
mod_gg1 <- data.frame(matrix(ncol = 3, nrow = nrow(CommuteTripsPerson)))
colnames(mod_gg1) <- c("TRPDUR", "INCOM", "GEND_F")
mod_gg1$TRPDUR <- CommuteTripsPerson$TRPDUR
mod_gg1$INCOM <- mean(CommuteTripsPerson$INCOM)
mod_gg1$GEND_F <- 1

mod_gg2 <- data.frame(matrix(ncol = 3, nrow = nrow(CommuteTripsPerson)))
colnames(mod_gg2) <- c("TRPDUR", "INCOM", "GEND_F")
...
colnames(pred_dat) <- c("TRPDUR", "Pred_GendF_Inc50K", "Pred_GendM_Inc50K")
pred_dat$TRPDUR <- CommuteTripsPerson$TRPDUR
pred_dat$Pred_GendF_Inc50K <- predict(mod_C, mod_gg1, type="response") #store predicted probabilities in new column
pred_dat$Pred_GendM_Inc50K <- predict(mod_C, mod_gg2, type="response")
...
ggplot(pred_dat_wd, aes(x = TRPDUR, y = value, colour = Scenario)) + 
  ...
  theme_bw()
```
Our model suggests that if income is constant, men are more likely to drive alone to work no matter the length of their trip. 

**Prediction Accuracy**
Calculating our mode's prediction accuracy gives us an accuracy rate of 86% - pretty good!

```{r prediction accuracy}
predC <- as.data.frame(fitted(mod_C))
predC <- rename(pred, "prob" = "fitted(mod_C)")
predC <- mutate(pred, "binary" = ifelse(prob < 0.5, 0, 1)) #if predicted probabilities are <0.5, then classify as zero, otherwise classify as 1
...
```
The coefficients of our model suggest that the odds ratio of a person driving alone to work is 39% *lower* if they are female, a unit increase in trip duration *decreases* the odds ratio of someone driving alone to work by 1%, and a unit increase in income *increases* the odds ratio of a person driving alone to work by 4%.
