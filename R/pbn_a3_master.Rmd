...
# Part B: Transportation Mode Choice with Logit Models

In the second part of this assignment, we use binomial and multinomial logit models to explore how a person's household, demographic, and trip characteristics affect their choice of transportation mode to work using data from the DVRPC Household Travel Survey (HHTS).

Section B.1 uses binomial logit models to explore the relationships between those who drive alone to work and household, demographic, and trip characteristics vs. those who do not drive alone to work.

Section B.2 also uses binomial logit models, but this time to compare walking and biking choice to other modes against similar characteristics.

Section B.3 uses multinomial logit models to explain whether someone drove, carpooled, took public transit, or walked/biked to work.
...
## B.1A Processing and Exploration

First, we create a dataframe that combines household and person level information.
...
head(per_hh_clean)
```
After selecting our variables of interest, we have a dataset with 10,391 observations and 13 variables.

Next we explore our trip data.

```{r explore transit mode data head}
head(trip_dat,10)
```
So many variables! Let's look at the breakdown of transmit modes for the first, second, and third transit modes respondents took to work.

```{r transit mode plot 1}
trip_dat %>% 
  filter(TRAN1 < 55) %>% 
ggplot( aes(x=TRAN1)) +
    geom_histogram( binwidth=3, fill="#6496b4", color="#0059c7", alpha=0.9) +
    ggtitle("TRAN 1 Mode Choice") +
    theme_bw()
```
```{r tran1 sum}
print("TRAN1 Summary")
summary(trip_dat$TRAN1)
```
Graphing and summarizing the first trip mode (`TRAN1`) shows that most people are driving themselves to work (code 21) but a noticeable amount of people are walking or biking (codes 11-14).

<br>

```{r transit mode plot 2}
trip_dat %>% 
    filter(TRAN2 < 60) %>% 
ggplot( aes(x=TRAN2)) +
    geom_histogram( binwidth=3, fill="#6496b4", color="#0059c7", alpha=0.9) +
    ggtitle("TRAN 2 Mode Choice") +
    theme_bw()
```
```{r tran2 sum}
print("TRAN2 Summary")
summary(trip_dat$TRAN2)
```
For mode 2, we see that more people are walking or biking and more people are taking transit (codes 40-53).

<br>

```{r transit mode plot 3}
trip_dat %>% 
    filter(TRAN3 < 60) %>% 
ggplot( aes(x=TRAN3)) +
    geom_histogram( binwidth=3, fill="#6496b4", color="#0059c7", alpha=0.9) +
    ggtitle("TRAN 3 Mode Choice") +
    theme_bw()
```

```{r tran3 sum}
print("TRAN3 Summary")
summary(trip_dat$TRAN3)
```
And we see that the third mode is more variable.

<br>

Next, we pull out those that drove alone to work during their first work trip and narrow the data down to our key variables of interest.
...
```
We see that many more people in this subset drove alone to work than walked or biked.

<br>

## B.1B Variable Selection

Now, we select the variables we are most interested in testing with our models.
...
We then test our variables of interest for colinearity with the dependent variable (shown in descending order of cor. score).

```{r Correlations with dependent variable, echo=FALSE}
...
This yields our final list of 11 independent variables to include in our models:
...
We develop a binomial logit model to to understand the factors that influence the binary outcome of whether or not someone drove alone to work and to make predictions about the probability of them doing so in the future based on our independent variables of interest.

Crucially, we are trying to determine which of these indepdendent variables have the most predictive relevance.

To do so, we first create a baseline model, called `mod_A`, that includes all of our variables of interest.
...
summary(mod_A)
```
We see that many of the variables included in `mod_A` lack significance but four of our predictor variables - `GEND_F`, `TRPDUR`, `INCOM`, `TRNCT` - appear to be significant predictors of driving alone to work.

The AIC is 2585. The null deviance is high at 2817.8, indicating a poor fit. But the residual deviance is lower at 2561.0, which indicates that the model explains a significant amount of the variance in the response variable.
...
```
Running `drop1` gives us some useful information. We see that the model with `GEND_F`, `TRPDUR`, `INCOM`, `TOLL`, and `TRNCT` as predictor variables are significant as they result in a significant increase in deviance and AIC values when dropped from the full model. 

Their large LRT and small p-value further indicate their significance.

Therefore, we run a new model with just `GEND_F`, `TRPDUR`, `INCOM`, `TOLL`, and `TRNCT` based on these results.
...
summary(mod_B)
```
Interestingly, in this new model, all of the variables are significant except `TOLL`. We also see that the AIC has increased by 38 and the residual deviance has increased by 50, indicating a slightly poorer fit.
...
summary(mod_C)
```
In this third model, all of the variables are significant, but the AIC has increased by 52 and the residual deviance increased by 54 from model 2, indicating a slightly worse fit.

## B1.D Model Selection

To determine which model to select as our final model, we compare the fit of the three models with ANOVA.

```{r Check fit with ANOVA}
anova(mod_A, mod_B, mod_C, test="Chisq")
```

The Analysis of Deviance table from ANOVA allows us to compare the fit of the three models against each other. The p-values indicate that both Model 2 and 3 are significantly better fits than Model 1, with Model 3 being the best fit since it gives the largest decrease in deviance. 

Because this model achieved this by removing the `PARKC`, `PARKU_H`, `PARKU_D`, `PARKU_W`, and `PARKU_M` variables, it is both our leanest, and meanest model.

```{r stargazer, results='asis'}
stargazer(mod_A, mod_B, mod_C, type="html", digits = 2, single.row = FALSE)
```
Comparing the models with a stargazer table helps us see that the AIC increase of 90 is comparitively slight, and worth the trade off of a model that has solely significant variables.

<br>

## B.1E Interpret Final Model Results
...
The chart below shows that even as trip duration increases, the odds ratio of a person driving alone to work continues to be lower if they are female.
...
In summary, these models suggest that the gender of a driver, the duration of their trip, their income, and the cost of using transit are the most important predictors of whether they will drive alone to work. These findings seem reasonable. In the U.S., women still share an unequal burden of childcare compared to men. Additionally, the gender pay gap could be a factor, since income appears to be a significant predictor of driving alone as well. It is intuitive that a longer trip could also encourage car-pooling or other modes, as it changes the cost-benefit ratio of driving alone or driving all together.

Our finding that higher transit cost decreases the odds ratio that a person will drive alone to work was surprising however. Intuitively, it would seem that cheap transit would increase the attractiveness of this mode. This finding suggests that future study of the variables and their interactions is necessary.

We found that the cost of parking, and whether commuters pay hourly, daily, weekly, or monthly are of secondary importance. It is not surprising that these supply-side factors had a secondary effect on the attractiveness of driving. In our correlation tests, it was interesting, to see that daily parking had a higher and positive correlation with driving alone than the other parking cost units (hourly, weekly, or monthly). This could be because daily parkers have more erratic commuting patterns than other commuters and therefore are more likely to drive alone when they do drive.

Finally, these models suggest that the age of a driver and the presence of a toll are of relatively little significance to whether or not they will drive alone. The age finding is surprising, as social patterns and tendencies change with age. Therefore, this suggests these models and variables would benefit from more lengthy development. Presence of a toll could not be significant simply because so many highways are toll roads in this region.
