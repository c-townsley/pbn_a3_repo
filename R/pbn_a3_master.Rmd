...
#let's grab anyone who has one of four modes in their sequence: 
#walked or bike/non-motorized
#drive alone
#carpooled
#transit
...
trip_dat$nonmoto[trip_dat$transit==1 & trip_dat$nonmoto==1]<-0
trip_dat$carpool[trip_dat$transit==1 & trip_dat$carpool==1]<-0

# and if carpool and drive alone conflict, give it to carpool (same for walk/bike and carpool)
trip_dat$drivealone[trip_dat$carpool==1 & trip_dat$drivealone==1]<-0
trip_dat$nonmoto[trip_dat$carpool==1 & trip_dat$nonmoto==1]<-0

#if walkbike and drive alone conflict, give it to drive alone
trip_dat$nonmoto[trip_dat$drivealone==1 & trip_dat$nonmoto==1]<-0
```

```{r MNL cleaning and processing}
...
trip_dat$nonmoto[which(is.na(trip_dat$nonmoto))]<-0

trip_dat$transit[which(is.na(trip_dat$transit))]<-0

##clean out any rows where they are all zero 
trip_dat$SUMZERO <- trip_dat$transit + trip_dat$nonmoto + trip_dat$drivealone + trip_dat$carpool
...
CommuteTripsClean$Commute[CommuteTripsClean$nonmoto == 1] <- 3
CommuteTripsClean$Commute[CommuteTripsClean$transit == 1] <- 4
...
#Some people take multiple work trips. Let's just grab the first work trip
CommuteTripsClean <- subset(CommuteTripsClean, !duplicated(SAMPN_PER))
```
...
per_datClean <- per_hh_dat %>% 
  select(-VETMO, -W2TY, -W2TYO, -W2TYP, -W2LOC, -W2IND, -W2INO, -W2OCC, -W2OCO, -W2DAY, -W2HOM,
          -W2HOO, -W2ST, -W2ET, -W1WKD3, -W1WKD4, -W1WKE, -W1WKD1, -W1WKD2)
...
CommuteTripsPerson$check <- 
  CommuteTripsPerson$transit +
  CommuteTripsPerson$drivealone + 
  CommuteTripsPerson$carpool + 
  CommuteTripsPerson$nonmoto
max(CommuteTripsPerson$check)
```
## Variable Selection

```{r MNL Variable Selection}
#keep only useful variables
CommuteTripsPerson <- CommuteTripsPerson %>% 
  dplyr::select(SAMPN.x, PERNO.x, AGE, GEND, INCOME, SAMPN_PER, TOLLA, TOLL, PARKC, PARKU, PRKUO, TRPDUR, drivealone, nonmoto, transit, carpool, Commute)

#Check for NAs
any(is.na(CommuteTripsPerson))

#convert all NAs to zero
CommuteTripsPerson[sapply(CommuteTripsPerson, is.na)] <- 0

#all good? - yes.
any(is.na(CommuteTripsPerson))

#and let's check for outliers
summary(CommuteTripsPerson)

#remove outliers
CommuteTripsPerson <-CommuteTripsPerson[which(CommuteTripsPerson$AGE %in% 1:110),]
CommuteTripsPerson <-CommuteTripsPerson[which(CommuteTripsPerson$GEND %in% 1:2),]
...
NonMoto<-CommuteTripsPerson[which(CommuteTripsPerson$nonmoto ==1),]
...
head(Carpool)
NonMoto$distance<-(12/60)*NonMoto$TRPDUR
head(NonMoto)
Transit$distance<-(25/60)*Transit$TRPDUR
head(Transit)

#re-combine the three data sets 
dat <- rbind(NonMoto, Transit, Drivealone, Carpool)
head(dat)
...
dat <- dat %>% 
  select(AGE, GEND, INCOME, time.auto, time.carpool, time.nonmoto, time.transit, mode)
head(dat)
...
#not in example code, maybe delete.
dat <- dat %>%
  mutate(mode = as.factor(mode))
  

head(dat)
datMNL <- mlogit.data(dat, shape="wide", choice="mode", varying=c(4:7))

datMNL_nopool <- mlogit.data(dat_nopool, shape="wide", choice="mode", varying=c(4:6))

#datMNL <- dfidx(dat, shape="wide", choice="mode", varying= 4:7, idnames = c("chid", "alt"))
