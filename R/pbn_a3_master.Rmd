...
trip_dat$walkbike[trip_dat$transit==1 & trip_dat$walkbike==1]<-0
...
trip_dat$walkbike[trip_dat$carpool==1 & trip_dat$walkbike==1]<-0

#if walkbike and drive alone conflict, give it to drive alone
trip_dat$walkbike[trip_dat$drivealone==1 & trip_dat$walkbike==1]<-0
```
...
trip_dat$walkbike[which(is.na(trip_dat$walkbike))]<-0

trip_dat$transit[which(is.na(trip_dat$transit))]<-0

##clean out any rows where they are all zero 
trip_dat$SUMZERO <- trip_dat$transit + trip_dat$walkbike + trip_dat$drivealone + trip_dat$carpool
...
CommuteTripsClean$Commute[CommuteTripsClean$walkbike == 1] <- 3
...
CommuteTripsPerson <- CommuteTripsPerson %>% 
  dplyr::select(SAMPN.x, PERNO.x, AGE, GEND, INCOME, SAMPN_PER, TOLLA, TOLL, PARKC, PARKU, PRKUO, TRPDUR, drivealone, walkbike, transit, carpool, Commute)
