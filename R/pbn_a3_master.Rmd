...
```{r ML_Model3}
Dat_E <- Dat_D %>% dplyr::select(-CITBORO_10, -DIST_PASSR, -DIST_4LNE_, -PAL_WETLND, -PCT_WHITE_, -PCT_SFHOME)

set.seed(3456)
trainIndex3 <- createDataPartition(Dat_E$DIST_INTER, p = .70, list = FALSE, times = 1)

Dat_E_Train <- Dat_E[ trainIndex3,]
Dat_E_Test  <- Dat_E[-trainIndex3,]

ML_Model3 <- glm(CHNG_URB ~ . - 1, family="binomial"(link="logit"), data = Dat_E_Train)
summary(ML_Model3)
```
