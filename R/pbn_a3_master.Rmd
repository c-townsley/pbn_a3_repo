...
trip_dat$nonmoto[which(is.na(trip_dat$nonmoto))] <-0 #Remove NAs
...
trip_dat$carpool[which(is.na(trip_dat$carpool))] <-0 #Remove NAs
...
trip_dat$transit[which(is.na(trip_dat$transit))] <-0 #Remove NAs
...
dat <-dat[which(dat$AGE %in% 1:110),]
...
varsInterest <- c("AGE", "GEND", "INCOME", "time.auto", "time.carpool", "time.nonmoto", "time.transit", "mode")
