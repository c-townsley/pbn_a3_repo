...
=======
Let's try this another way. What happens if we let the machine do the work of sorting out what variables to include in the model?
```{r training_set}
>>>>>>> 39e23ed257f8536c0df63191867d8d6374f9f894
set.seed(3456)
...
!!!!!!!!!!!!!!!!!!!!!!!
First, let's make a new dataset with all our chosen values for analysis.
```{r new dataset with analysis variables}
CalFish_Clean <-  CalFish_Clean %>% 
  mutate(Inundation_Bi_Fac = as.factor(Inundation_Bi)) %>%
  select(DistRiver, Greyness, Slope, Elevation, FlowAcc, Inundation_Bi, DistRiverCutoff, ElevationCutoff, Inundation_Bi_Fac)

CalFish_Clean_NoGeo <-  st_set_geometry(CalFish_Clean, NULL)

CalFish_Clean_Analysis <-  CalFish_Clean_NoGeo %>%
  select(DistRiver, Greyness, Slope, FlowAcc, ElevationCutoff, Inundation_Bi_Fac)
```

Now,let's Partition Training and Test Data
```{r training_set}
set.seed(3456)
trainIndex <- createDataPartition(CalFish_Clean_Analysis$FlowAcc, p = .70,
                                  list = FALSE,
                                  times = 1)

FloodTrain <- CalFish_Clean_Analysis[ trainIndex,]
FloodTest  <- CalFish_Clean_Analysis[-trainIndex,]
```

Train Model
```{r Train tModel, warining = FALSE, message = FALSE}
# Train the model
FloodModel <- glm(Inundation_Bi_Fac ~ ., family = "binomial"(link = "logit"), data = FloodTrain)
summary(FloodModel)
```
...
=======
Dat_A_Train <- Dat_A[ trainIndex,]
Dat_A_Test  <- Dat_A[-trainIndex,]
```

```{r firstModel, warining = FALSE, message = FALSE}
MechModel <- glm(CHNG_URB ~ ., 
                    family="binomial"(link="logit"), data = Dat_A_Train)
summary(MechModel)
```
>>>>>>> 39e23ed257f8536c0df63191867d8d6374f9f894

```{r confusion_matrix, message = FALSE, warning = FALSE}
classProbs <- predict(MechModel, Dat_A_Test, type="response")

testProbs <- data.frame(obs = as.numeric(Dat_A_Test$CHNG_URB), pred = classProbs)
...
Binomial Dependent Variable: Drove to work or got to work any other way.
