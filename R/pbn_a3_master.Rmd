...
ML_Model3 <- glm(CHNG_URB ~ . + 0, family="binomial"(link="logit"), data = Dat_D_Train)
summary(ML_Model3)
```
...
ML_Model4 <- glm(CHNG_URB ~ . + 0, family="binomial"(link="logit"), data = Dat_E_Train)
summary(ML_Model4)
```

Compare Results of ALL Models
```{r stargazer}
stargazer(ML_Model2, type="text", digits = 2, single.row = FALSE)
```
...
Model4_Acc
```
From this summary of accuracy tests, it appears that Model 1 is the most accurate, at ~68%.
...
# B.1 Driving to Work vs. Other Modes
...
CommuteTripsClean2 <- CommuteTripsClean %>% 
  select(SAMPN, PERNO, SAMPN_PER, TOLLA, TOLL, PARKC, PARKU, TRPDUR, drivealone)
...
The binomial Dependent Variable for our model is `drivealone`: Which is whether the person drove to work vs. got to work any other way.
...
* `TOLLA`: toll amount
* `PARKC`: parking cost ($)
* `PARKU`: parking cost unit (per hour, day, week, month, or other) **Needs to be dummy variable**
...
print("Gender")
cor(CommuteTripsPerson$drivealone, CommuteTripsPerson$GEND, use="complete.obs", method="pearson")
print("Age")
...
mod_A_vars <- CommuteTripsPerson %>% 
  dplyr::select(drivealone, TOLL, TOLLA, TRPDUR, INCOM, GEND, AGE, PARKC)
...
## B.1C Binomial Logit Model

```{r binomial logit model 1}
mod_A <- glm ( drivealone ~ AGE+GEND+TRPDUR+INCOM+TOLLA+TOLL+PARKC, data=CommuteTripsPerson, family = binomial)
summary(mod_A)
```
